...
import {
  ...
  Heart, Filter, Search, Verified, Camera
} from 'lucide-react';
...
const MarketplacePage: React.FC = () => {
  ...
  const addToCart = (item: Homestay | Product | Event, type: 'homestay' | 'product' | 'event', details?: any) => {
    const cartItem: CartItem = {
      id: item.id,
      type,
      title: type === 'homestay' ? (item as Homestay).name : (item as Product | Event).title,
      price: type === 'event' ? (item as Event).ticketPrice : (item as Homestay | Product).price,
      image: item.image,
      quantity: 1,
      details,
